...
class GameController {
	...
	async gameRequestList(req, res) {
		const { _id } = req.user;
		const gamesId = await GameModel.find({ userId: _id });
	...
	async gameCreate(req, res) {
		const { _id } = req.user;
		const { name, startTime, resultTime, endTime, status, userId } = req.body;
	...
	async updateGame(req, res) {
		...
		const storeData = await GameModel.findOneAndUpdate(
			{ _id: new mongoose.Types.ObjectId(id) },
			{ name, resultTime, startTime, endTime, status },
		);
		return res.json({
			status: true,
		...
		if (gameRequests.length > 0) {
			for (const iterator of gameRequests) {
				...
				await useData.updateOne({
					wallet:
						Number(useData.wallet) +
						Number(iterator.gameNumber[0].price * 90),
				});
			}
		}
		return res.json({
			...
			if (gameRequests.length > 0) {
				for (const _user of gameRequests) {
					...
					let updateUser = await useData.updateOne({
						wallet:
							Number(useData.wallet) +
							Number(_user.gameNumber[0].price * 90),
					});
				}
