...
class GameController {
  ...
  async gameList(req, res) {
    ...
    for (const iterator of list) {
      ...
      let gameStartDateTime = moment(
        moment(new Date()).format("DD-MM-Y") + " " + iterator.startTime,
        "DD-MM-Y HH:mm"
      );
      // let gameEndDateTime = moment(moment(new Date()).format("DD-MM-Y") + " " + game.endTime, "DD-MM-Y HH:mm").add(1, 'day');
      let gameEndDateTime = moment(
        moment(new Date()).format("DD-MM-Y") + " " + iterator.endTime,
        "DD-MM-Y HH:mm"
      );
  ...
  async gamePlay(req, res) {
    try {
      ...
      let currentDateTime = moment(new Date());
      let gameStartDateTime = moment(
        moment(new Date()).format("DD-MM-Y") + " " + game.startTime,
        "DD-MM-Y HH:mm"
      );
      // let gameEndDateTime = moment(moment(new Date()).format("DD-MM-Y") + " " + game.endTime, "DD-MM-Y HH:mm").add(1, 'day');
      let gameEndDateTime = moment(
        moment(new Date()).format("DD-MM-Y") + " " + game.endTime,
        "DD-MM-Y HH:mm"
      );
